...
Pipeline* gf3d_pipeline_fullscreen_create(VkDevice device, char* vertFile, char* fragFile, VkExtent2D extent, Uint32 descriptorCount) {
    ...
    VkPipelineVertexInputStateCreateInfo emptyInputState = { 0 };
    ...
    emptyInputState.pVertexAttributeDescriptions = NULL;
    emptyInputState.vertexBindingDescriptionCount = 0;
    emptyInputState.pVertexBindingDescriptions = NULL;
    ...
    //Comment these out to get rid of descriptor sets
    //gf3d_pipeline_create_basic_model_descriptor_pool(pipe);
    //f3d_pipeline_create_basic_model_descriptor_set_layout(pipe);
    //gf3d_pipeline_create_descriptor_sets(pipe);

    //descriptor pool here
    int i;
    ...
    memcpy(&binding[0], &uboLayoutBinding, sizeof(VkDescriptorSetLayoutBinding));
    ...
    pipelineLayoutInfo.pSetLayouts = &pipe->descriptorSetLayout; // descriptorSet goes here
    pipelineLayoutInfo.pushConstantRangeCount = 0; // Optional
    pipelineLayoutInfo.pPushConstantRanges = NULL; // Optional

    gf3d_pipeline_render_pass_setup(pipe); //Think about gutting it to only stuff referenced in the triangle tutorial (make new function)
...
//Borrowing DJ's Sprite code to act as the base of the UI. Will alter and tweak as needed
Pipeline* gf3d_pipeline_basic_sprite_create(VkDevice device, char* vertFile, char* fragFile, VkExtent2D extent, Uint32 descriptorCount)
{
    ...
    colorBlending.sType = VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO; //DONO
    colorBlending.logicOpEnable = VK_FALSE;
    colorBlending.logicOp = VK_LOGIC_OP_COPY; // Optional
    colorBlending.attachmentCount = 1;
    colorBlending.pAttachments = &colorBlendAttachment;
